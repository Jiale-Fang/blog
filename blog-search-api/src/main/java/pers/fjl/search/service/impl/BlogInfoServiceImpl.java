package pers.fjl.search.service.impl;
...
import pers.fjl.search.dao.BlogInfoMapper;
import pers.fjl.search.pojo.BlogInfo;
import pers.fjl.search.service.BlogInfoService;
...
//        SearchRequest searchRequest = new SearchRequest("blog-search");
        ...
        highlightBuilder.field("description").field("title");
        ...
        for (SearchHit hit : searchResponse.getHits().getHits()) {  //hits中的字段没有被高亮，因此要将es返回highlight中的高亮字段替换到hit中
            ...
            Map<String, Object> sourceAsMap2 = replaceHits(description, sourceAsMap1, "description");
    ...
    public int total(String queryString, boolean flag) throws IOException {
        ...
        if (flag) {
            // 精准匹配
            MultiMatchQueryBuilder multiMatchQueryBuilder = QueryBuilders.multiMatchQuery(queryString, "title", "content", "description", "type_name", "tag_name");
            sourceBuilder.query(multiMatchQueryBuilder);
        }
        sourceBuilder.timeout(new TimeValue(60, TimeUnit.SECONDS));
        // 执行搜素
