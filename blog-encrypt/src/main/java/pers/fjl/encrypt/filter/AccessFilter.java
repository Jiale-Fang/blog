        ...
        try {
            ...
            if (list == null) {
                ...
                if (list != null && list.size() > 0) {
                    redisUtil.set(CacheKey.RESOURCE, list, 3600);
                }
            }
            ...
            if (count == 0) {
                ...
                errorResponse("这是未授权的请求", 401, 200);
            }

            //6.取出当前用户拥有的权限资源表，判断当前用户是否有权限访问当前请求
            DecodedJWT verify = getTokenInfo(token);
            Long uid = Long.parseLong(verify.getClaim("id").asString());
            List<String> userResource = (List<String>) redisUtil.get(String.valueOf(uid));
            if (userResource == null) {
                //缓存中不存在
                userResource = resourceApi.getUserResource(String.valueOf(uid));
                if (userResource != null && userResource.size() > 0) {
                    redisUtil.set(String.valueOf(uid), userResource, 1800);
                }
            }
            ...
            if (count > 0) {
                ...
                errorResponse("当前用户没有权限", 401, 200);
                return null;
            }

        } catch (Exception e) {
            errorResponse("解析token出错", 500, 200);
            e.printStackTrace();
            return null;
        }
    }
